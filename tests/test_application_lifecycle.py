...
def test_lifecycle_tracks_connected_bots_and_runs_connection_hooks() -> None:
    ...
    async def on_connect(bot: object) -> None:
        calls.append(("connect", cast("FakeBot", bot).self_id))

    async def on_disconnect(bot: object) -> None:
        calls.append(("disconnect", cast("FakeBot", bot).self_id))
...
def test_lifecycle_runs_first_connection_hooks_only_once() -> None:
    ...
    async def on_first_connect(_bot: object) -> None:
        calls.append("first")

    async def on_connect(_bot: object) -> None:
