...
DETAIL_AND_ARTICLE_REQUEST_COUNT = 2
...
def test_article_dynamic_detail_is_completed_from_article_body() -> None:
    requests: list[httpx.Request] = []
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            200,
            json={"code": 0, "data": {"content": "完整专栏正文" * 500}},
        )
    ...
    response = asyncio.run(fetch())
    article = response.data["data"]["item"]["modules"]["module_dynamic"][  # type: ignore[index]
        "major"
    ]["article"]

    assert len(requests) == DETAIL_AND_ARTICLE_REQUEST_COUNT
    assert str(requests[1].url).startswith(ARTICLE_DETAIL_URL)
    assert requests[1].url.params["id"] == "52875386"
    assert article["desc"] == "完整专栏正文" * 500
    assert article["_ironsbot_body_hydrated"] is True
