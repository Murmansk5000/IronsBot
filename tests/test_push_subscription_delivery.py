    ...
    summary = asyncio.run(
        delivery.send_targets(
            [
                ...
                OneBotMessageTarget("group", 2001),
                OneBotMessageTarget("group", 2002),
            ],
            "推送正文",
            bot=bot,
...
def test_send_target_messages_does_not_share_mutated_message_between_targets(
    ...
    assert bot.private_messages == [(1001, "机器人已开启。\n\n回复 TD 管理私聊推送。")]
...
def test_send_target_messages_appends_subscription_hint_once_per_day(
    ...
    asyncio.run(
        delivery.send_targets(
            [OneBotMessageTarget("private", 1001), OneBotMessageTarget("group", 2001)],
            "第一次",
            bot=bot,
            subscription_key="startup_notice",
        )
