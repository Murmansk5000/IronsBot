...
def test_cached_score_candidate_page_starts_uses_facts_and_score_bounds() -> None:
    ...
    def get_cache_summary(**_kwargs: Any) -> list[SimpleNamespace]:
        return [
            SimpleNamespace(
                start_index=82,
                end_index=89,
...
def test_cached_score_miss_boundary_uses_complete_cached_page_gap() -> None:
    ...
    def get_cached_page_result(**_kwargs: Any) -> SimpleNamespace:
        return SimpleNamespace(
            fetched_at=FETCHED_AT,
            items=[
                SimpleNamespace(id=1, nick="A", score=110),
                SimpleNamespace(id=2, nick="B", score=HIGHER_SCORE),
                SimpleNamespace(id=3, nick="C", score=LOWER_SCORE),
                SimpleNamespace(id=4, nick="D", score=90),
            ],
        )
