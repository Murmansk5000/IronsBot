...
**未知配置可见忽略（2026-09-22）：** 根据生产维护策略，TOML 中无法识别或已经
删除的字段恢复为“记录完整配置路径 warning 后忽略”，但不恢复旧字段别名、迁移器、
双读或兼容语义；字段不会生效。已知字段的类型、范围、必填项、密钥位置和跨配置引用
继续严格校验，错误仍阻止启动。加载器先用 `extra=forbid` 只收集未知路径，再用
`extra=ignore` 生成正式模型，因此未知字段不会掩盖同一文件中的真实配置错误。
