...
`seerapi/scripts/build_seerapi_data_db.py` 当前仍有 2,780 行（本 Spec 开始时为 3,098 行）。它同时拥有网络请求、官方
...
| Remaining published-table writers | 写入魂印、群星牌、伙伴和 metadata 表 | 创建连接、确定发布顺序 | 写入前后表内容与 metadata 不变；writer 不读取环境或网络 | planned |
...
Phase 4 [████░░░░░░]  verified work: manifest/effect/new-content boundaries plus two SQLite writers
Current  [██████████]  completed: ConfigPackage and official reference tables moved to dedicated SQLite writers
```
