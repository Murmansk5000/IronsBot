...
| Requester targeting | Text payload transport | Reference the triggering message from its inbound `msg_idx`; send one minimal group-scoped Markdown mention before the plain-text body on the first reply only | AppID, group and member scope are validated before serialization; exact-digest client evidence contains one native `at`, one body and no literal protocol tag; later stages do not repeat the mention or exceed the five-reply budget |
...
| 2026-09-20 | Isolated requester mention and body | Exact runtime `851bf5a9` / `sha256:08978df9...67188`; workflow `35494510291`; NapCat group history; QQ client observation; full suite `3843 passed, 7 skipped` | The first reply payload contains only `reply + markdown + at`; the second contains one TEXT body. The requester is mentioned once, the body appears once, and no literal OpenID protocol tag is exposed. Later progress, result and media stages do not repeat the mention, preserving the five-reply budget. |
