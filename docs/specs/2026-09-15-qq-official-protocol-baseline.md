...
| Requester targeting | Text payload transport | Reference the triggering message from its inbound `msg_idx` and prepend a group-scoped member mention at the official transport boundary | AppID, group and member scope are validated before serialization; exact-digest client evidence contains both the native reply and target-consistent Tencent mention URI without a literal protocol tag |
...
| 2026-09-20 | Scoped requester mention implementation | Current official text-interaction document; same-target production TEXT/MARKDOWN probe; exact runtime `6d56bed64` / `sha256:fc400ae9...c493`; NapCat group history; QQ Official renderer and transport tests | TEXT leaked literal markup. MARKDOWN produced an independent `at` segment for the short probe and a target-consistent `mqqapi://markdown/mention` for the long help reply. The formal command replied once, retained native source reference, and leaked no literal tag. Group replies therefore remain passive Markdown with scoped mentions; C2C remains TEXT and cross-group mentions fail before transport. |
