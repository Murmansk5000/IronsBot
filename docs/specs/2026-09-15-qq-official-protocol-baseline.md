...
| 2026-09-15 | Automated closure run | Full suite `3454 passed, 7 skipped`; Ruff; production and test BasedPyright; compileall; static repository checks; clean tracked worktree | All local gates pass. The remaining manual Tencent account matrix below must not be reported as passed without captured platform evidence |
...
| Access token refresh | Keep the account active through refresh or force the SDK cache to expire, then perform an authenticated API call | Redacted `access token refreshed` lifecycle event followed by a successful API operation | pending real refresh evidence |
