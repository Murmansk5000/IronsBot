...
| `ironsbot/services/seer/rendering/templates/**` | **GPL-3.0-or-later** | Seer 渲染模板、图片和 CSS 资产。 |
| `ironsbot/integrations/headless_seer/**` | **GPL-3.0-or-later** | 见 [ironsbot/integrations/headless_seer/LICENSE](ironsbot/integrations/headless_seer/LICENSE)，另见同目录 `NOTICE` |
| 其余全部代码（其他插件、`ironsbot/integrations`、应用入口、`docker/` 等） | **MIT** | 见根目录 [LICENSE](LICENSE) |

GPL-3.0 全文见 [LICENSE.GPL-3.0](LICENSE.GPL-3.0)。

## 这意味着什么

- **单独复用非插件模块**：你可以单独提取以 MIT 标注的模块（例如 `ironsbot/integrations/seer_data` 等），按 MIT 协议在你自己的项目中使用，无需承担 GPL 义务（可闭源、可商用）。
