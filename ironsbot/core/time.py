...
class ScheduledClockTime:
    ...
    @classmethod
    def parse(cls, value: object, *, error_message: str) -> ScheduledClockTime:
...
def normalize_daily_time(value: object, *, error_message: str) -> str:
    """Normalize an existing minute-precision configuration value."""
    text = str(value).strip()
    parts = text.split(":")
    if len(parts) != TIME_PART_COUNT:
        raise ValueError(error_message)
    clock_time = scheduled_clock_time(value, error_message=error_message)
    return f"{clock_time.hour:02d}:{clock_time.minute:02d}"
...
def normalize_daily_time_with_seconds(
    ...
    *,
    ...
    """Normalize minute or second precision input to canonical HH:MM:SS."""
