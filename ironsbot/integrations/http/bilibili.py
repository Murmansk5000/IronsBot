...
async def fetch_bili_dynamic_detail(
    ...
    data: Any = response.json()
    article_id = _dynamic_article_id(data)
    if article_id is not None:
        return await _complete_article_body(
            client,
            cookie,
            dynamic_id,
            article_id,
            BiliFeedResponse(response.status_code, data),
        )
    ...
    if opus_response.status_code == HTTP_OK and _api_code(opus_data) == 0 and opus_body:
...
async def _complete_article_body(
    ...
    dynamic_id: str,
    article_id: int,
    detail_response: BiliFeedResponse,
) -> BiliFeedResponse:
    try:
        ...
        article_data: Any = article_response.json()
    except Exception:
        logger.exception(
            "Bilibili article body completion request failed: dynamic=%s article=%s",
            dynamic_id,
            article_id,
        )
        return detail_response

    article_body = _article_body(article_data)
    if (
        article_response.status_code == HTTP_OK
        and _api_code(article_data) == 0
        and article_body
    ):
        data = _replace_article_description(detail_response.data, article_body)
    else:
        logger.warning(
            "Bilibili article body completion unavailable: dynamic=%s article=%s "
            "http=%s code=%s",
            dynamic_id,
            article_id,
            article_response.status_code,
            _api_code(article_data),
        )
        data = detail_response.data
    return BiliFeedResponse(detail_response.status_code, data)
...
def _dynamic_article_id(payload: object) -> int | None:
    ...
    article = major.get("article") if isinstance(major, Mapping) else None
...
def _article_body(payload: object) -> str:
    if not isinstance(payload, Mapping):
        return ""
    data = payload.get("data")
...
def _replace_article_description(payload: object, body: str) -> object:
    resolved = deepcopy(payload)
    if not isinstance(resolved, dict):
        return payload
    data = resolved.get("data")
