...
def boost_slots_at(
    ...
    """Find configured release-burst slots matching this exact second."""
    ...
    for index, window in enumerate(polling.boost_windows):
        ...
        if offset not in window.offset_seconds:
            continue
        slots.append(BoostSlot(index, anchor + timedelta(seconds=elapsed - offset)))
    return tuple(slots)
...
def _boost_slot_seconds(window: BiliBoostWindow) -> tuple[int, ...]:
    error_message = "bilibili.polling.boost_windows time must use HH:MM:SS"
    start_second = second_of_day(window.start, error_message=error_message)
    end_second = second_of_day(window.end, error_message=error_message)
    duration = (end_second - start_second) % _SECONDS_PER_DAY
    if duration == 0:
        return ()
    interval_seconds = window.interval_minutes * 60
