...
def dynamic_delivery_content(item: dict[str, Any]) -> str:
    if dynamic_body_hydration_reason(item) in {"truncated", "truncated_article"}:
        return "正文暂未获取完整，请打开动态链接查看。"
    return dynamic_content(item)
