...
超级管理员。跨平台身份关联尚未开放；目标流程由 NapCat 侧已知的数字 QQ 发起一次性
签名链接或按钮，再由官方机器人事件中的 OpenID 用户点击确认。用户无需手输验证码；
按钮能力不可用时才降级为短期验证码。程序不会用昵称、头像、消息时间或
`union_openid` 猜测数字 QQ，关联必须支持撤销并按 AppID 与身份作用域隔离。
