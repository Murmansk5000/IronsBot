...
| Seer request-scheduler requester attribution | transition | `PlayerRequestProtectionService` currently accepts an integer OneBot user ID for priority, pause bypass, workflow telemetry and semantic tracing | Move requester identity to `ActorRef` end-to-end. The feature policy may adapt a OneBot actor to configured superuser state, but Seer and queue services must not accept QQ integers. |
| Headless-operation actor/conversation diagnostics | transition | `HeadlessOperationTracker` still records integer `group_id` and semantic integer user metadata for legacy diagnostics | Store typed `ActorRef` / `ConversationRef` in operation traces and render platform-specific diagnostics only at the notification adapter boundary. |
