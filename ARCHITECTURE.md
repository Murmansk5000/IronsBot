...
`ironsbot.plugins.messaging.meeting`: the manifest loads them directly, and
each package supplies its own metadata and contribution. `sendpic` and
`meeting` also own the command descriptors for the matchers they install.
